...
void HardwareAbstractionLayer::discoverHardware() {
    ...
    std::cout << "HAL: Starting hardware discovery..." << std::endl;
    ...
    // 2. GPU Detection (Simulated env check)
    const char* cuda = std::getenv("CUDA_VISIBLE_DEVICES");
    const char* rocm = std::getenv("ROCM_PATH");

    if (cuda) {
        m_devices.push_back(std::make_shared<SimulatedDevice>(
            DeviceCapability{"NVIDIA GeForce GPU (CUDA)", DeviceCategory::GPU, "NVIDIA", 12288, 12000, 5888, 98.0f, true, true}
        ));
    }
    if (rocm) {
        m_devices.push_back(std::make_shared<SimulatedDevice>(
            DeviceCapability{"AMD Radeon GPU (ROCm)", DeviceCategory::GPU, "AMD", 16384, 16000, 3840, 95.0f, true, true}
        ));
    }

    // 3. NPU Detection (Simulated)
    m_devices.push_back(std::make_shared<SimulatedDevice>(
        DeviceCapability{"Integrated AI NPU", DeviceCategory::NPU, "Generic", 0, 0, 1, 90.0f, true, true}
    ));

    std::cout << "HAL: Discovery complete. Found " << m_devices.size() << " hardware platforms." << std::endl;
}
